...
parser = argparse.ArgumentParser(
    description="Modify FITS headers.",
...
parser.add_argument(
    "fixesfile", type=str, action="store", help="Name of parameter file."
)
parser.add_argument(
    "-n",
    action="store_true",
...
if not os.path.exists("original-headers"):
    if args.n:
        print(
            "I would have created the original-headers directory to backup files."
        )
    else:
...
for row in ascii.read(
    args.fixesfile, format="csv", delimiter=",", comment="#", fast_reader=False
):
    for i in range(row["first frame"], row["last frame"] + 1):
        file_tests = [
            "lmi.{:04d}.fits".format(i),
        ...
        if not any(test):
            ...
            backup = "original-headers/{}".format(fn).replace(
                ".fits", ".header"
            )
            if not os.path.exists(backup):
                if args.n:
                    print(
                        "I would have saved a backup header:"
                        " {} -> {}".format(fn, backup)
                    )
                else:
                    print(
                        "Backup original FITS header: {} -> {}".format(
                            fn, backup
                        )
                    )
