...
for f in $* ; do
    ...
    if echo $imagetype|grep -q OBJECT; then
		...
		solve-field --no-verify --use-sextractor --skip-solved --scale-low=$scalelow --scale-high=$scalehigh --scale-units=arcsecperpix -t0 --radius=0.5 --ra=$ra --dec=$dec $g --depth=50
